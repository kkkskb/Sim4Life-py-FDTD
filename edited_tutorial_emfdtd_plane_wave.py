...
def AnalyzeSimulation(sim):
    ...
    # Connect E-field component to the viewer (f0 is the center frequency)
    slice_field_viewer_efield.Inputs[0].Connect( overall_field_sensor['EM E(x,y,z,f0)'] )
    # Set data display mode and component
    ...
    # Set slice plane to YZ
    slice_field_viewer_efield.Slice.Plane = slice_field_viewer_efield.Slice.Plane.enum.YZ
    # Update viewer and go to max slice
    slice_field_viewer_efield.Update(0)
    slice_field_viewer_efield.GotoMaxSlice()
    # Add viewer as an algorithm to the S4L document
    document.AllAlgorithms.Add( slice_field_viewer_efield )

# --- ここから、元のRunTutorial関数 (変更なしで残します) ---
def RunTutorial():
    """
    Original RunTutorial function from the script.
    This function is not directly used in the new multiple simulation workflow,
    but is kept to maintain the original code structure.
    """
    import s4l_v1.document
    
    s4l_v1.document.New() # Create a new document

    CreateModel() # Create model entities
    
    #sim = CreateSimulation() # Create a single simulation (calls original CreateSimulation)
    
    sim = _create_single_simulation_instance('Plane Wave Simulation', 90.0, 270.0) # Create a single simulation with default angles
    
    s4l_v1.document.AllSimulations.Add(sim) # Add simulation to document
...
def main(data_path=None, project_dir=None):
    ...
    # Calls the newly added function to run multiple simulations.
    #RunMultiplePlaneWaveSimulations()

    # --- Original Single Simulation Run (commented out and preserved) ---
    RunTutorial() 

    # --- Original commented-out block for project saving is preserved ---
    # Uncomment and enable saving functionality if needed.
    """
